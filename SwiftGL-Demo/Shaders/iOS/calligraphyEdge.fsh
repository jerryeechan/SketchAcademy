...
void main()
{
    ...
    vec4 textureColor = texture2D(texture, rotated);
    if(textureColor.r == 1.0||destColor.r ==1.0)
    {
        ...
        gl_FragColor.g = 1.0;
        gl_FragColor.b = 1.0;
        gl_FragColor.a = textureColor.a;// = vec4(0.0,0.0,0.0,0.0);//destColor * (1.0 - texture2D(texture, rotated).a * 0.1);
    }
    else
    {
