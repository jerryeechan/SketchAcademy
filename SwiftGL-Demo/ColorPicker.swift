...
@IBDesignable public class ColorPicker: UIView {
    ...
    private func writeColorData(d:UnsafeMutablePointer<UInt8>) {
        ...
        var q:CGFloat = 0.0
        var t:CGFloat = 0.0
        let sd:CGFloat = 1.0 / bounds.width
        let vd:CGFloat =  1 / bounds.height
    ...
    private func dataToPickerImage(d:[UInt8],  inout pickerImage:PickerImage, width:UInt, height:UInt) {
        ...
        let cgimg = CGImageCreate(Int(width), Int(height), Int(8), Int(32), Int(width) * Int(4),
            colorSpace, bitmapInfo, pickerImage.provider!, nil as  UnsafePointer<CGFloat>, true,CGColorRenderingIntent.RenderingIntentDefault)
